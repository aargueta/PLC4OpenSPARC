...
static void return_fp_packet(struct pcx_pkt *pcx_pkt, struct cpx_pkt *cpx_pkt,
...
void process_fp_1(struct pcx_pkt *pcx_pkt, struct cpx_pkt *cpx_pkt) {
	...
	case FP_OPCODE_FDTOS:
		...
		default:
			printf("ERROR: unknown single operand floating point "
...
void process_fp_2(struct pcx_pkt *pcx_pkt, struct cpx_pkt *cpx_pkt) {
	...
	case FP_OPCODE_FCMPS:
		...
		if (float32_is_signaling_nan(fp_rs1_float) || float32_is_signaling_nan(
				fp_rs2_float)) {
			float_exception_flags |= float_flag_invalid;
		}
		break;
	...
	case FP_OPCODE_FCMPED:
		...
		break;
	default:
		printf("ERROR: process_fp_2(): unknown two operand floating "
